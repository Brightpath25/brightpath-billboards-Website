...
export default function EventCard({ title, description, timeframe, href, subLinks }: EventCardProps) {
  return (
    <article style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif" }} className="group relative flex h-full flex-col overflow-hidden border-t border-white/15 bg-[#101210] p-7 transition-colors hover:border-[#e3b04b] sm:p-8">
      <div className="absolute right-0 top-0 h-full w-1 bg-[#e3b04b]/0 transition-colors group-hover:bg-[#e3b04b]" aria-hidden="true" />
      <p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.22em] text-[#e3b04b]">{timeframe}</p>
      <h3 className="mb-4 max-w-md text-2xl font-semibold leading-tight tracking-normal text-[#f4f1e8] md:text-3xl" style={{ fontFamily: "'Gill Sans', 'Gill Sans Nova', 'Trebuchet MS', 'Helvetica Neue', Arial, sans-serif" }}>{title}</h3>
      <p className="mb-7 flex-grow text-base leading-7 text-[#aeb5bd]">{description}</p>
